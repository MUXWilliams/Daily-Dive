...
def test_the_readme_names_the_current_publication():
    """The README sat titled "Daily Dive" for weeks after the rename, telling
    anyone who found the repo that this was a morning digest. Same drift as
    site/about.html — prose in a file nobody re-reads."""
    readme = Path("README.md").read_text(encoding="utf-8")
    assert readme.splitlines()[0] == f"# {brand.PUBLICATION}"
    assert brand.SITE_NAME in readme

    # The repo name and the bot are deliberately NOT renamed, so a blanket
    # search for the old name would be wrong. These are the claims that were
    # actually false.
    for stale in ("morning digest", "every morning", "one scannable page every morning"):
        assert stale not in readme, stale


def test_the_readme_points_at_files_that_exist():
    """A front page that links to a moved file spends a reader's trust before
    they find out — the same test CLAUDE.md already has."""
    readme = Path("README.md").read_text(encoding="utf-8")
    for path in re.findall(r"\]\(([a-zA-Z0-9_./-]+\.(?:py|md|toml|yml|json|html))\)", readme):
        assert Path(path).exists(), path
    for path in re.findall(r"`([a-zA-Z0-9_./-]+\.(?:py|md|toml|yml|json|sqlite3))`", readme):
        if path.endswith(".sqlite3"):
            continue  # generated by a run, absent on a clean checkout
        assert Path(path).exists() or (Path("dailydive") / path).exists(), path


def test_the_readme_source_count_matches_sources_toml():
    """A number in prose is a claim, and this one rots every time a feed is
    added. If it drifts, either fix the number or stop printing one."""
    readme = Path("README.md").read_text(encoding="utf-8")
    enabled = len([s for s in config.load_sources(Path("sources.toml"))])

    claimed = re.search(r"\*\*(\d+) live sources\*\*", readme)
    assert claimed, "the README no longer states a source count — remove this test too"
